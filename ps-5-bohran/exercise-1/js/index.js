...
function renderTrack(track){
    let parent = $("#records");
    let image = `<img src="${track.artworkUrl100}" alt = "${track.trackName}" title = "${track.trackName}">`;
    parent.append(image);   
}
...
function renderSearchResults(tracks) {
    ...
    $('#records').empty(); 
    tracks.results.forEach(function(item){
        renderTrack(item);
    });
    } else {
...
// encoded data.
// 
const URL_TEMPLATE = "https://itunes.apple.com/search?entity=song&limit=25&term="; // eslint-disable-line

function fetchTrackList(term) {
    let url = (URL_TEMPLATE + term);
    toggleSpinner();
...
function renderError(error){
    let message = `<p class="alert alert-danger">${error.message}<p>`;
