...
describe('Has required HTML', () => {
    ...
    test('Includes page title', () => {
        ...
        expect(title.text()).not.toEqual("Document");
    })
    ...
    test('Has an image', () => {
        ...
        expect(img.attr('alt').length).toBeGreaterThanOrEqual(1);
        expect(img.attr('alt').toLowerCase()).not.toMatch(/picture\/*/);
        expect(img.attr('alt').toLowerCase()).not.toMatch(/image\/*/);
    })
    ...
    test('List has at least 3 items', () => {
        ...
        //no empty items!
        let empty = li.filter(function (i, elem) {
...
describe('Has required CSS', () => {
    ...
    test('Body has default font family', () => {
        ...
        let withoutCharacters = family.replace(/"/g, "")
            .replace(/'/g, "")
            .replace(/, /g, ',')
        expect(withoutCharacters).toMatch(/Helvetica Neue,Helvetica,Arial,sans-serif/)
    })
