...
describe("3. Media queries for small sized screens (598px or greater)", () => {
    let mediaRules = cssObj.stylesheet.rules.filter((d) => d.type === 'media' && d.media.replace(/ /g, '').indexOf("(min-width:598px)") !== -1)[0].rules
...
describe("4. Media queries for medium sized screens (768px or greater)", () => {
    let mediaRules = cssObj.stylesheet.rules.filter((d) => d.type === 'media' && d.media.replace(/ /g, '').indexOf("(min-width:768px)") !== -1)[0].rules
    test('Media rules are present', () => {
    ...
    test('Text shadow is added to `main` element', () => {
        ...
        expect(mainRules.declarations[0].value.toLowerCase()).toEqual('1px 1px #153c43')
    })
})

describe("5. Media queries for large sized screens (992px or greater)", () => {
    let mediaRules = cssObj.stylesheet.rules.filter((d) => d.type === 'media' && d.media.replace(/ /g, '').indexOf("(min-width:992px)") !== -1)[0].rules
    test('Media rules are present', () => {
...
describe("6. Media queries for extra-large sized screens (1200px or greater)", () => {
    let mediaRules = cssObj.stylesheet.rules.filter((d) => d.type === 'media' && d.media.replace(/ /g, '').indexOf("(min-width:1200px)") !== -1)[0].rules
    // Background image
    test('Background image is updated', () => {
        let bodyRules = mediaRules.filter((d) => d.selectors.indexOf('body') !== -1)[0];
        let backgroundImageRule = bodyRules.declarations.filter((d) => d.property === 'background-image')[0]
