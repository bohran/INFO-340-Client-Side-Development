...
/* Create and log a variable `twelveString` that is the result of passing
    the character sting "8" (in quote) to your addFour() function. 
...
function compoundInterest(initial, rate, years) {
    let compound = initial * Math.exp(rate * years);
    return compound;
}
let solution = compoundInterest(1000, .06, 5);
...
function getHighCard(card) {
    let val = card[0];
    let highVal = card[0].rank;
    for (let i = 0; i < card.length; i++) {
        if (card[i].rank > highVal) {
            highVal = card[i].rank;
            val = card[i];
        }
    }
