...
let array = lost.map(extractOpponent); 
console.log(array);
...
lost.forEach(function(item) {
        console.log(item.opponent + " at " + item.home + ", " + item.opponent_score + " to " + item.home_score);
});
...
//Log out HOW MANY games included fumbles.
 let oneFumble = huskyGames2016.filter(function(item){
    return item.fumbles >= 1; 
    }
 );
 console.log(oneFumble.length);
